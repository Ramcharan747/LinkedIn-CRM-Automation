...
class BotController:
    ...
    def check_daily_limits(self, df):
        ...
        # We handle NaN/Empty values and missing columns safely
        if 'Timestamp_Last_Action' in df.columns:
            valid_dates = df['Timestamp_Last_Action'].astype(str)
            actions_today = df[valid_dates.str.contains(today_str, na=False)]
        else:
            actions_today = pd.DataFrame()
        
        connects_today = len(actions_today[actions_today['State'] == 'Sent']) if not actions_today.empty else 0
        followups_today = len(actions_today[actions_today['State'] == 'Completed']) if not actions_today.empty else 0
    ...
    def get_next_job(self):
        ...
        if limits['can_connect']:
            new_leads = df[df['State'].isin(['Idle', '', ' ']) | df['State'].isna()]
